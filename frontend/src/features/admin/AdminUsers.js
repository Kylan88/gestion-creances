...
const AdminUsers = () => {
  ...
  const fetchUsers = useCallback(async (page = 1, filters = {}) => {
    ...
    try {
      ...
      setTotalPages(response.data.total_pages || 1); // Assumer que backend renvoie total_pages si paginé
    } catch (err) {
      console.error('Erreur lors de la récupération des utilisateurs:', err);
      const errMsg = err.response?.data?.error || 'Erreur lors de la récupération des utilisateurs';
      setError(errMsg);
      toast.error(errMsg);
    } finally {
      setLoading(false);
  ...
  }, [currentPage, searchUser, roleFilter, dateDebut, dateFin, fetchUsers, user]);

  const handleSearch = (e) => {
    e.preventDefault();
    setCurrentPage(1);
    fetchUsers(1, { search_user: searchUser, role: roleFilter, date_debut: dateDebut, date_fin: dateFin });
  };

  const handleRoleChange = async (userId, newRole) => {
    if (newRole === 'admin' || newRole === 'user') {
      try {
        await api.post('/admin/users', { user_id: userId, role: newRole });
        toast.success(`Rôle mis à jour pour l'utilisateur.`);
        fetchUsers(currentPage, { search_user: searchUser, role: roleFilter, date_debut: dateDebut, date_fin: dateFin });
      } catch (err) {
  ...
  const formattedUsers = useMemo(() => 
    users.map(u => ({
      ...u,
      formattedDate: new Date(u.created_at || Date.now()).toLocaleString('fr-FR', {
        year: 'numeric',
        month: 'short',
        day: 'numeric'
      })
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <p className="text-gray-300 font-medium">Chargement des utilisateurs...</p>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <header className="mb-8 text-center relative">
        ...
        <p className="text-gray-400 text-sm">Administration des comptes et rôles</p>
      </header>
      ...
      <AnimatePresence>
        {error && (
          ...
          >
            {error}
            <Button onClick={() => fetchUsers(1, {})} variant="ghost" size="sm" className="mt-2 text-red-200 hover:text-red-100">
              Réessayer
            </Button>
          </motion.div>
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white/10 border-b border-gray-600/50">
                  ...
                  <th className="px-6 py-4 text-left text-gray-300 font-semibold">Nom d'utilisateur</th>
                  ...
                  <th className="px-6 py-4 text-left text-gray-300 font-semibold">Date Création</th>
              ...
              <tbody className="divide-y divide-gray-600/30">
                <AnimatePresence>
                  {formattedUsers.map((u) => (
                    ...
                    >
                      <td className="px-6 py-4 text-gray-100">{u.id}</td>
                      <td className="px-6 py-4 text-gray-100">{u.username}</td>
                      ...
                      <td className="px-6 py-4 text-gray-300">{u.formattedDate}</td>
                      <td className="px-6 py-4">
                        <select
                          value={u.role}
                          onChange={(e) => handleRoleChange(u.id, e.target.value)}
                          className="bg-white/10 text-gray-100 border border-gray-600/50 focus:border-emerald-400 rounded px-2 py-1 text-xs"
                        >
                ...
                {formattedUsers.length === 0 && !loading && (
                  <motion.tr
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  >
